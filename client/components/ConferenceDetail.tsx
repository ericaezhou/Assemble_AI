...
export default function ConferenceDetail({ conferenceId, userId, onBack }: ConferenceDetailProps) {
  ...
  const getRecommendedParticipants = () => {
    ...
    const sorted = [...otherParticipants].sort((a, b) => (b.similarity_score || 0) - (a.similarity_score || 0));
  ...
  const recommendedParticipants = getRecommendedParticipants();
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-7xl mx-auto p-5 md:p-10 space-y-8">
        ...
        {recommendedParticipants.length > 0 && (
          <div>
            <h2 className="text-2xl font-bold text-gray-800 mb-5">Recommended to Meet</h2>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
              {recommendedParticipants.map(participant => (
                <ResearcherCard key={participant.id} researcher={participant} />
              ))}
            </div>
          </div>
        )}
